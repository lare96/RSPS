		...
		switch (id) {
			...
			case DialogueConstants.OPTIONS_3_1:
				if (player.getInventory().hasItemId(ZAMORAKIAN_SPEAR)) {
					if (player.isPouchPayment() && player.getMoneyPouch() > CREATION_COST) {
						player.setMoneyPouch(player.getMoneyPouch() - CREATION_COST);
					} else if (player.getInventory().hasItemAmount(995, CREATION_COST)) {
