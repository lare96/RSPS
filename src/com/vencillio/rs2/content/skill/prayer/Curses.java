...
public class Curses {
	...
	public void applySoulSplit(final Mob npc, int damage) {//PvE
		...
		if (p.getSkill().getLevels()[3] < maxHp) {
			...
			int offX = (pY - oY) * -1;
			int offY = (pX - oX) * -1;
			//p.getPA().createPlayersProjectile(pX, pY, offX, offY, 50, 25, 2263, 15, 10, -p.oldNpcIndex - 1, 0);
			...
			TaskQueue.queue(new Task(0) {
				public void execute() {
					...
					int offX2 = (oY - pY) * -1;
					int offY2 = (oX - pX) * -1;
					//p.getPA().createPlayersProjectile(oX, oY, offX2, offY2, 50, 25, 2263, 15, 10, -p.playerId - 1, 0);
